...
//      changeGameplayMode(GameplayMode.LEVEL_COMPLETED); // for Cutscene debug
        ...
        if (ghostMode == GhostMode.FRIGHTENED
            ...
            for (Ghost ghost : ghosts) {
                ...
                if (ghost.getMode() != GhostMode.EATEN
                    ...
                    // and mode of each ghost is any of EATEN, IN_PEN, LEAVING_PEN, RE_LEAVING_FROM_PEN, or ENTERING_PEN,
                    // it is not updated.
    ...
    private void figureOutPenLeaving() {
        ...
        if (alternatePenLeavingScheme) {
            // By using number of dots eaten after a level restart,
    ...
    public void dotEaten(int[] dotPos) {
        ...
        if (getPathElement(dotPos[1], dotPos[0]).getDot() == Dot.ENERGIZER) { // when eating an energizer
            ...
            addToScore(50);
            SokPacman.publish(context, "dotEaten",
                "isEnergizer",
                "true",
                "DotsRemaining",
                String.valueOf(getPlayfieldEl().getDotsRemaining()),
                "DotsEaten", 
                String.valueOf(getPlayfieldEl().getDotsEaten()));
        } else { // when eating normal food
            addToScore(10);
            SokPacman.publish(context, "dotEaten",
                "isEnergizer",
                "false",
    ...
    private void changeGameplayMode(GameplayMode mode) {
        ...
        case PLAYER_DYING:
            ...
            SokPacman.publish(context, "PACMAN_DYING",
                "DotsRemaining",
                String.valueOf(getPlayfieldEl().getDotsRemaining()),
                "DotsEaten", 
                String.valueOf(getPlayfieldEl().getDotsEaten()),
        ...
        case GAME_RESTARTING:
            ...
            SokPacman.publish(context, "GAME_RESTARTING");
            break;
        case GAME_RESTARTED:
            stopAllAudio();
            canvasEl.setVisibility(true);
        ...
        case NEWGAME_STARTING:
            ...
            SokPacman.publish(context, "NEWGAME_STARTING");
            break;
        case NEWGAME_STARTED:
            ...
            break;
        case GAMEOVER:
            SokPacman.publish(context, "GAMEOVER");
        case KILL_SCREEN:
            removeReadyElement();
            stopAllAudio();
            createGameOverElement();
            gameplayModeTime = timing[9];
            SokPacman.publish(context, "KILL_SCREEN");
            break;
        case LEVEL_BEING_COMPLETED:
            stopAllAudio();
            gameplayModeTime = timing[10];
            SokPacman.publish(context, "LEVEL_BEING_COMPLETED");
            break;
        ...
        case GHOST_DIED:
            ...
            SokPacman.publish(context, "GHOST_DIED");
            break;
        case CUTSCENE:
            startCutscene();
            break;
